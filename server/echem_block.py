...
def multi_dqdv_plot(df, cycle_list, 
					...
					for cycle in half_cycles:
						try:
							...
							myvoltage, dqdv, cap = ec.dqdv_single_cycle(df_cycle[capacity_label], 
							...
							x_volt = np.linspace(min(df_cycle[voltage_label]), max(df_cycle[voltage_label]), num=int(1e4))
...
class CycleBlock(DataBlock):
	...
	def plot_cycle(self, voltage_label='Voltage', 
		...
		if self.data['plotmode-dqdv']: 
			...
			dqdv_df = send_bokeh_dqdv(df, cycle_list,
			polynomial_spline=a, 
			s_spline= 10 ** (-b), 
			win_size_1=c, 
			win_size_2=d )
			...
			layout = bokeh_plots.double_axes_plot(df, df2=dqdv_df, y_default="Voltage", x_axis_label="Capacity", x="dqdv" )
		...
		#If user has activated dvdq mode
			...
			dvdq_df = send_bokeh_dvdq(df, cycle_list,
			polynomial_spline=a, 
			s_spline= 10 ** (-b), 
			win_size_1=c, 
			win_size_2=d )
