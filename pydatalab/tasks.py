...
def collect_plugin_panels(_):
    """Discover Vue panel components from installed pydatalab plugins and wire
    ...
    and before building the webapp.
    """
    import importlib
    ...
    def _write_index(path: pathlib.Path, registered: dict[str, str]) -> None:
        lines = [
            "// Auto-generated by `invoke dev.collect-plugin-panels` — do not edit manually.",
            "// Run `uv run invoke dev.collect-plugin-panels` after installing or updating plugins.",
    ...
    # webapp/src lives beside pydatalab/ at the repository root; resolve via
    # __file__ so the task works regardless of the invoking directory.
    src_path = pathlib.Path(__file__).resolve().parent.parent / "webapp" / "src"
    if not src_path.exists():
        raise SystemExit(f"Could not find webapp/src at {src_path}.")
    ...
    for ep in eps:
        ...
        webapp_dir = pathlib.Path(mod.__file__).parent / "webapp"
