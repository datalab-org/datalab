...
def test_get_sample_data(client, default_sample_dict):
    ...
    for key in default_sample_dict.keys():
        if isinstance(v := default_sample_dict[key], datetime.datetime):
            v = v.isoformat()
        assert response.json["sample_data"][key] == v
...
def test_new_sample_collision(client, default_sample_dict):
    ...
    assert response.json["message"] == "sample_id_validation_error"
...
def test_save_bad_sample(client, default_sample_dict):
    ...
    assert response.json["message"] == "Unable to update sample '12345' with new data."
...
def test_delete_sample(client, default_sample_dict):
    response = client.post(
        "/delete-sample/",
        json={"sample_id": default_sample_dict["sample_id"]},
    )
    assert response.status_code == 200
    assert response.json["status"] == "success"
    ...
    response = client.get(
        f"/get_sample_data/{default_sample_dict['sample_id']}",
    )
