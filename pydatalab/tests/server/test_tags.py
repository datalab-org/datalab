...
- **user**: a personal tag owned by exactly one user; only that user can list,
...
def test_create_personal_tag(client, unauthenticated_client, user_id):
    """Any logged-in user can create a personal tag they own; the default scope is
    ...
    # A normal user can create a personal tag (scope defaults to "user").
...
def test_personal_tag_visibility(client, another_client, admin_client, user_id):
    """A personal tag is listed/searchable only by its owner."""
...
def test_scope_based_name_uniqueness(client, another_client, admin_client):
    ...
    # A global and a personal tag may share a name.
    ...
    # Two different users may each own a personal "flammable".
    assert _create_tag(another_client, "flammable").status_code == 201

    # The same owner cannot create a second personal tag with the same name.
...
def test_patch_tag_owner_only(client, another_client, admin_client):
    """Personal tags are editable only by their owner; global tags only by admins."""
    personal_id = _create_tag(client, "editable", description="first").json["data"]["immutable_id"]
    global_id = _create_tag(admin_client, "global-editable", scope="global").json["data"][
        "immutable_id"
    ]

    # Another user cannot edit someone else's personal tag.
    assert (
        another_client.patch(
            f"/tags/{personal_id}", json={"data": {"description": "nope"}}
        ).status_code
        == 403
    )
    ...
    # The owner can.
    assert (
        client.patch(f"/tags/{personal_id}", json={"data": {"description": "updated"}}).status_code
        == 200
    )
    patched = next(t for t in client.get("/tags").json["data"] if t["immutable_id"] == personal_id)
    ...
    assert (
        client.patch(f"/tags/{personal_id}", json={"data": {"scope": "global"}}).status_code == 200
    )
    still = next(t for t in client.get("/tags").json["data"] if t["immutable_id"] == personal_id)
...
def test_patch_tag_rename_unique_within_scope(client):
    """Renaming a personal tag onto another of the owner's names is rejected."""
...
def test_delete_tag_owner_only(client, another_client, admin_client, database):
    """Personal tags are deletable only by their owner; global only by admins; refs
    are pulled from items on delete."""
    personal_id = _create_tag(client, "deletable", color="#abcdef").json["data"]["immutable_id"]
    ...
    assert (
        client.post(
            ...
            json={
                "item_id": "tag-delete",
                "data": {"tags": [{"type": "tags", "immutable_id": personal_id}]},
            },
        ).status_code
        == 200
    )

    # Another user cannot delete someone else's personal tag.
    assert another_client.delete(f"/tags/{personal_id}").status_code == 403

    # An admin cannot delete another user's personal tag either (admins manage global).
    assert admin_client.delete(f"/tags/{personal_id}").status_code == 403

    # The owner can.
    assert client.delete(f"/tags/{personal_id}").status_code == 200
    ...
    assert client.delete(f"/tags/{personal_id}").status_code == 404
...
def test_cannot_add_another_users_personal_tag(
    client, another_client, admin_client, database, another_user_id
):
    """A user may add global and own personal tags, but not another user's personal
    tag; they may keep and remove an already-present foreign tag."""
    a_tag = _create_tag(client, "a-personal").json["data"]["immutable_id"]
    b_tag = _create_tag(another_client, "b-personal").json["data"]["immutable_id"]
    ...
    # B can add a global tag and their own personal tag.
    ...
    # B cannot *introduce* A's personal tag (a new addition B does not own).
    assert save(another_client, [ref(g_tag), ref(b_tag), ref(a_tag)]).status_code == 403
    # The rejected save left the stored tags unchanged.
    assert set(_item_tag_ids(another_client, "shared-item")) == {g_tag, b_tag}

    # A (the owner) can add their own personal tag; the item now carries it.
...
def test_cannot_add_foreign_tag_on_creation(client, another_client):
    """Personal tags owned by another user are rejected at item creation too."""
...
def test_item_tag_resolution_includes_scope(
    ...
    sees the owner's personal tag (display is gated by the item, not the tag)."""
    personal_id = _create_tag(client, "resolve-personal", color="#abcdef").json["data"][
        "immutable_id"
    ]
    ...
    save = client.post(
        ...
        json={
            ...
            "data": {
                "tags": [
                    {"type": "tags", "immutable_id": personal_id, "name": "stale"},
    ...
    assert tags[personal_id]["scope"] == "user"
    assert tags[personal_id]["name"] == "resolve-personal"  # re-resolved, not stale
    assert tags[personal_id]["color"] == "#abcdef"
