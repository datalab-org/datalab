...
def test_server_authoritative_fields_survive_saves_of_referenced_blocks(
    sample_with_block, admin_client, database
):
    """`metadata`/`computed` are server-authoritative: they are stripped from any
    web payload and must be restored from the *stored* block. For a referenced
    block that stored payload lives in the `blocks` collection, not in the item's
    `blocks_obj` entry, so both /save-item/ and /update-block/ have to resolve it
    before rebuilding the block.
    """
    sample_id, _, block_id = sample_with_block
    ...
    # Stand in for fields that only block code or block events can write
    database.blocks.update_one(
        {"_id": immutable_id},
        {"$set": {"data.metadata": {"instrument": "abc"}, "data.computed": {"peak": 1.23}}},
    )

    # Saving the item round-trips the block through `from_web`, which must not
    # drop the fields the client never received
    item_data = _get_item_data(admin_client, sample_id)
    item_data["blocks_obj"][block_id]["freeform_comment"] = "edited via save-item"
    _save_item(admin_client, sample_id, item_data)

    block_doc = database.blocks.find_one({"_id": immutable_id})
    assert block_doc["data"]["freeform_comment"] == "edited via save-item"
    assert block_doc["data"]["metadata"] == {"instrument": "abc"}
    assert block_doc["data"]["computed"] == {"peak": 1.23}

    # ...and neither may the dedicated block-update route
    block_data = _get_item_data(admin_client, sample_id)["blocks_obj"][block_id]
    block_data["freeform_comment"] = "edited via update-block"
    response = admin_client.post("/update-block/", json={"block_data": block_data})
    assert response.status_code == 200, response.json

    block_doc = database.blocks.find_one({"_id": immutable_id})
    assert block_doc["data"]["freeform_comment"] == "edited via update-block"
    assert block_doc["data"]["metadata"] == {"instrument": "abc"}
    assert block_doc["data"]["computed"] == {"peak": 1.23}
...
def test_block_write_permissions(
    sample_with_block,
    admin_client,
    another_client,
    database,
    ...
    """Block writes are authorized via the parent item. Since #1951 read access
    to the item is enough to update its blocks, and that must hold for a
    referenced block just as it did for an embedded one; a user with no access
    at all is still refused, as is a request without an `item_id`. Deletion
    ...
    item_data = _get_item_data(admin_client, sample_id)
    block_data = item_data["blocks_obj"][block_id]
    block_data["freeform_comment"] = "group edit"
    ...
    # to) read access, so this write is permitted and must land on the block
    # document rather than being silently dropped.
    response = another_client.post("/update-block/", json={"block_data": block_data})
    assert response.status_code == 200, response.json
    assert database.blocks.find_one({"_id": immutable_id})["data"]["freeform_comment"] == (
        "group edit"
    )

    # A user with no access to the item at all still cannot write its blocks
    private_id = f"test_block_sep_perm_{random_string[:8]}"
    private_sample = default_sample_dict.copy()
    private_sample["item_id"] = private_id
    private_sample.pop("group_ids", None)
    assert admin_client.post("/new-sample/", json=private_sample).status_code == 201

    response = admin_client.post(
        "/add-data-block/", json={"block_type": "comment", "item_id": private_id, "index": 0}
    )
    assert response.status_code == 200, response.json
    private_block_id = response.json["new_block_obj"]["block_id"]

    private_block_data = _get_item_data(admin_client, private_id)["blocks_obj"][private_block_id]
    private_block_data["freeform_comment"] = "sneaky edit"
    response = another_client.post("/update-block/", json={"block_data": private_block_data})
    assert response.status_code == 404

    private_immutable_id = database.items.find_one({"item_id": private_id}, {"blocks_obj": 1})[
        "blocks_obj"
    ][private_block_id]["immutable_id"]
    private_doc = database.blocks.find_one({"_id": private_immutable_id})
    assert private_doc["data"].get("freeform_comment") is None

    admin_client.post("/delete-sample/", json={"item_id": private_id})
    ...
    # Deleting is creator-only, so read access does not extend to it
