...
def get_sample_data(sample_id):
    ...
    return jsonify(
        {"status": "success", "sample_id": sample_id, "sample_data": doc, "files_data": files_data}
    )
...
def delete_file_from_sample():
    ...
    updated_file_entry = FILE_COLLECTION.find_one_and_update(
        {"_id": file_id}, {"$pull": {"sample_ids": sample_id}}, return_document=ReturnDocument.AFTER
    )
    ...
    return (
        jsonify(
            {"status": "success", "new_file_obj": {request_json["file_id"]: updated_file_entry}}
        ),
        200,
    )
