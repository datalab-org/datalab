...
class EntryReference(BaseModel):
    ...
    def check_id_fields(cls, values):
        """Check that only one of the possible identifier fields (any field but type in this class or subclasses."""

        id_fields = [f for f in cls.schema()["properties"] if f != "type"]
