...
export default function setupUppy(sample_id, trigger_selector, reactive_file_list) {
	...
	uppy.use(Dashboard, {
		...
		metaFields: [
			{ id: 'blahblah', name: 'Blah', placeholder: 'Metadata can be added here'}
		],
	})
	...
	uppy.on('file-added', (file) => {
		...
		for (const file_id in reactive_file_list) {
			...
			if (reactive_file_list[file_id].name == file.name) {
				...
				alert("A file with this name already exists in the sample. If you upload this, it will update the current file.")
			}
		}
	...
	uppy.on('complete', function (result) {
		...
		result.successful.forEach(function(f) {
			...
			store.commit('updateFiles', {[response_body.file_id]: response_body.file_information})
			console.log("Added file to store. Response_body:")
			console.log(response_body)
			if (!response_body.is_update) {
				store.commit('addFileToSample', {sample_id: sample_id , file_id: response_body.file_id}) 
			}
		});
	});
}
