import { getPictogramsFromHazardInformation, getPictogramsFromHazardCode } from "@/resources.js";
...
describe("test hazard pictograms", () => {
  ...
  function checkGHS(input, result, expected) {
    ...
    if (!expectedSet.has("NaN")) {
      expectedSet.delete("NaN");
    }

    if (actual.size !== expectedSet.size) {
      throw new Error(
        `Size mismatch: ${actual.size} vs ${expectedSet.size}: ${JSON.stringify(result)} vs ${expected} for ${input}`,
      );
    }
  ...
  it("parses various hazard statement strings into the correct codes", () => {
    let hazard_statements = [
      ...
      "Australian Department of Health: H319 H315 H360\nSigma-Aldrich: H315 H319 H335 H360\nEuropean Chemicals Agency: H315 H319 H335",
      "European Commission: H225 H351 H319 H335 Sigma-Aldrich: H225 H319 H335 H351 Fisher Scientific: H225 H319 H351 H335 European Chemicals Agency: H225 H319 H335 H351 European Chemicals Agency: H225 H319 H335 H351 European Chemicals Agency: H225 H319 H335 H351 H400",
      "This stuff is very dangerous",
      "This stuff is very dangerous H110",
    ];

    let expected_h_codes = [
      [200, 302, 402],
      [350, 341, 360, 334, 317, 410],
      [225],
      [315, 319, 335, 360],
      [225, 351, 319, 335],
      [225, 351, 319, 335, 351, 400],
      [],
      [110],
    ];
    ...
    for (const i in hazard_statements) {
      ...
      const result = getPictogramsFromHazardInformation(hazard_statement);
      expect(result.length).to.equal(expected.length);

      for (const code of expected) {
        expect(result.map((p) => p.code)).to.include(code);
      }
    }
  });
});
